...
根据上述内容可知多个调度表的执行模式：多个调度表可并发执行，调用 StartScheduleTable 启动 A、B 两个调度表后，各自时钟开始计时，当 A 调度表中有到期点触发后，调用优先级最高的任务执行。如果 B 调度表中任务到期，且比 A 调度表中正执行的任务优先级高，就会发生抢占。如果调度表 C 需要在 B 完成后执行，则需要设置同步点，在 B 完成后启动调度表 C。
